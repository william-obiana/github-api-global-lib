...
def call(String PYTHON_VERSION, String TEST_DIR, String REQUIREMENTS_FILE, String PYTEST_ARGS, String TARGET = "/tmp/target") {
    ...
    sh "pytest ${TARGET}/${TEST_DIR}/${PACKAGE_FILE} --junitxml=reports/${env.BUILD_ID}report.xml ${PYTEST_ARGS}" // run pytest and generate reports

    // define a reports map for the report.xml
    def reports = junit testResults: 'reports/${env.BUILD_ID}report.xml'
    step([$class: 'JUnitResultArchiver', testResults: 'reports/${env.BUILD_ID}report.xml']) // use JUnitResultArchiver Plugin to archive
}